...
$sq="select count('po.pin_code') as ptotal,pin_code from post_off_detail_table po left join state st on po.state_id=st.state_id left join city_table ct on ct.city_id=po.city_id left join post_office_table pt on pt.post_off_id=po.post_off_id where po.pin_code='".$pincode."' "; 
  $row1=mysql_query($sq);
  $re=mysql_fetch_array($row1);
  
  $sql="select * from post_off_detail_table po left join state st on po.state_id=st.state_id left join city_table ct on ct.city_id=po.city_id left join post_office_table pt on pt.post_off_id=po.post_off_id where po.pin_code='".$pincode."' "; 
$row=mysql_query($sql);  

?>   
		   
		    <span class="black_font">List of Post Offices with PinCode <?php echo $re['pin_code']; ?> <br /> Total <?php echo $re['ptotal']; ?> Post Offices found</span>
			<div>
			<?php
...
$re1=$res['state_name'];
 ?>                             
	 ...
	 ?>
	   <div>
							  <?php echo "You Have Intered Wrong Pin Code"; ?>
							  </div>
							  <?php } ?>       
