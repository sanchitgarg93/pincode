...
<div class="header">  <?php
 ...
 ?>
  ...
  <?php
		   ...
		   <?php 
		   $alphabat=$_GET['alpha'];
		  $sq="select ct.city_name from post_off_detail_table pt left join post_office_table po on pt.post_off_id=po.post_off_id left join state st on st.state_id=pt.state_id left join city_table ct on ct.city_id=pt.city_id where ct.city_id='".$_GET['value']."' order by po.post_off_name ASC";  
		   $ro=mysql_query($sq);
		   $rest=mysql_fetch_array($ro);
		  
	 	   $sql="select * from post_off_detail_table pt left join post_office_table po on pt.post_off_id=po.post_off_id left join state st on st.state_id=pt.state_id left join city_table ct on ct.city_id=pt.city_id where pt.city_id='".$_GET['value']."' and  po.post_off_name  LIKE '$alphabat%' order by po.post_off_name ASC"; 
$row=mysql_query($sql);
?>  
