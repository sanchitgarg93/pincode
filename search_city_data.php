...
<script>(function(d, s, id) {
 ...
 $sql="select * from post_off_detail_table po left join state st on po.state_id=st.state_id left join city_table ct on ct.city_id=po.city_id left join post_office_table pt on pt.post_off_id=po.post_off_id where po.post_off_id='".$_GET['city']."' "; 
$row=mysql_query($sql);  
if(mysql_num_rows($row)>=1)
{
$res=mysql_fetch_array($row);
$re=$res['pin_code'];
$re1=$res['post_off_name'];
 ?>                             
                              <div>
							  <table width="500">
	...
	while($rest=mysql_fetch_array($row1))
	{
							...
							<tr><td><?php  echo '<a href="pin_detail.php?value='.$rest["pin_code"].'">'.'Click Here to See Post Office With Same Pin Code'.'</a>'.'&nbsp;';  ?></td></tr>
